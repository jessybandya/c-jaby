...
export default function SimpleBeam({ simpleBendingFun }) {
  ...
  return (
    <Card>
      ...
      onClick={handleShow}
      style={{marginTop:8}}
      >Select Concrete Member(Cantiliver)</MDButton>
      <MDButton
      rel="noreferrer"
      variant="gradient"
      color={sidenavColor}
      style={{marginLeft:5, marginTop:8}}
      >Select Steel Member(Cantiliver)</MDButton>
      </center> 
      </div>
      <div></div>    
      </div>
      ...
      <Modal.Header style={{display:'block'}}>
         <div style={{alignItems:'center',justifyContent:'space-between',display:'flex'}}><MDTypography>CANTILIVER DESIGN</MDTypography> <ClearIcon fontSize='medium' style={{color:'#88888888',cursor:'pointer'}} onClick={handleClose}/></div>
         <div>
